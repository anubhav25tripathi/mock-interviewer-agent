...
const AuthForm = ({type}:{type:FormType}) => {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    try{
       if(type==='sign-up'){
        toast.success('Account was created successfully. Please sign in.');
        router.push('/sign-in')
       }
       else{
  ...
  return (
    <div className="card-border lg:min-w-[566px]">
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 w-full mt-4 form">
        ...
        <FormField 
         control={form.control} name="email" label="Email" placeholder="Your Email" type="email" />
         <FormField 
         control={form.control} name="password" label="Password" placeholder="Your Password" type="password" />
